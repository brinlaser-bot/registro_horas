...
import type { DayResult } from "@/lib/time";
import type { UserSettings } from "@/lib/storage";
import type { EntryType } from "@/lib/time";
import { formatMinutes, nowTimeString } from "@/lib/time";
import { Badge, Button, Card } from "@/components/ui";
import { useToast } from "@/components/toast";
import { addEntry as storageAdd, deleteEntry as storageDelete } from "@/lib/storage";
...
export function QuickPunch({ today, todayStr, settings, onDone }: Props) {
  ...
  const punch = (type: EntryType, t?: string) => {
    if (busy) return;
    setBusy(true);
    try {
      storageAdd({ date: todayStr, time: t ?? time, type, note: note.trim() || null });
      setNote("");
      toast.show(type === "entrada" ? "Entrada registrada!" : "Saída registrada!");
      onDone?.();
    } catch {
      toast.show("Não foi possível registrar.", "error");
    } finally {
      setBusy(false);
    }
  };
    ...
    >
      <div className="grid gap-4 sm:grid-cols-[auto_1fr]">
        <div className="flex items-center gap-4 rounded-xl border border-slate-100 bg-slate-50/70 p-4">
          ...
          <div>
            ...
            <p className={`mt-0.5 text-xs font-bold ${today.balanceMinutes >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
        ...
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
            <Button variant="primary" size="lg" loading={busy} onClick={() => punch("entrada")} className="w-full">
              <LogIn size={18} /> Entrada
            </Button>
            <Button variant="secondary" size="lg" loading={busy} onClick={() => punch("saida")} className="w-full border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100">
      ...
      {today.entries.length > 0 && (
        <div className="mt-4 border-t border-slate-100 pt-4">
          <div className="flex flex-wrap items-center gap-2">
            {today.entries.map((e) => (
              <span key={e.id} className="group inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white py-1 pl-3 pr-1.5 text-xs font-semibold text-slate-700 shadow-sm">
                ...
                <button onClick={() => remove(e.id)} className="rounded-full p-1 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500" aria-label="Remover">
                  <Trash2 size={12} />
                </button>
              </span>
            ))}
